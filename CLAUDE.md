...
- `src/ollama/` — Ollama provider (M11): self-hosted embeddings (`/api/embed`) + streaming generation (`/api/chat`) against a configurable host (`ollama_host` / `OLLAMA_HOST`, default `http://127.0.0.1:11434`), with model preflight via `/api/tags`. The host is auto-added to the egress allowlist. Used by `embed`/`generate` for both CLI and `serve`. See `docs/ollama.md`
